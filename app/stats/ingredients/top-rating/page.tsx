        ...
        const load = async () => {
            ...
            `)
                ...
                (data ?? []).forEach((row: any) => {
                    ...
                    const rating = row.pizzas?.rating as number | null;

                    if (!ingId || !ingName || typeof rating !== 'number') return;

                    if (!map[ingId]) {
                        map[ingId] = { name: ingName, sumRating: 0, ratingCount: 0 };
                    }
                    map[ingId].sumRating += rating;
                    map[ingId].ratingCount += 1;
                });

                const lb: LeaderboardRow[] = Object.entries(map)
                    .map(([id, v]) => ({
                        ingredientId: Number(id),
                        name: v.name,
                        avgRating: v.sumRating / v.ratingCount,
                        pizzaCount: v.ratingCount,
                    }))
                    .filter(row => row.pizzaCount >= 5)   // soglia minima
            ...
            <div className="flex-1 px-4 py-4 max-w-4xl mx-auto w-full flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                    <h1 className="text-sm font-semibold">
                        Classifica ingredienti – voto medio
                    </h1>
                    <p className="text-[11px] text-slate-400">
                        Ingredienti ordinati per voto medio delle pizze in cui compaiono, limitatamente al periodo selezionato ({periodLabel}). Sono inclusi solo ingredienti presenti in almeno cinque pizze.
                    </p>
                </div>
                                    ...
                                    >
                                        ...
                                        <div className="text-right">
                                            <span className="block text-sm font-semibold">
                                                ⭐ {row.avgRating.toFixed(2).replace('.', ',')}
                                            </span>
