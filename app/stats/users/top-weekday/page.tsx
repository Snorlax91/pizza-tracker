    ...
    useEffect(() => {
        const loadPizzas = async () => {
            ...
            try {
                ...
                // prendiamo tutte le pizze del periodo (user_id + eaten_at)
                ...
                // conteggio pizze per utente
                const counts: Record<string, number> = {};

                (data ?? []).forEach((row: any) => {
                    const uid = row.user_id as string;
                    const eaten_at = row.eaten_at as string | null;
                    if (!uid || !eaten_at) return;

                    const d = new Date(eaten_at);
                    if (Number.isNaN(d.getTime())) return;

                    if (d.getDay() !== weekday) return; // solo il giorno selezionato
